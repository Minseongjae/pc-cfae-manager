...
import { getShiftCardHeightFromShift, getScheduleCompactCardHeight } from '@/lib/shiftUtils';
...
export function getShiftCardStyle(
  ...
  const cardHeight = stacked
    ? getScheduleCompactCardHeight()
    : getShiftCardHeightFromShift(shift, compact);
